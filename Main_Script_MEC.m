...
% Dm = 100*1024*8; % output data size
...
%% Plotting Results 
x = 10+delta*(start:iter); % X axis of the curves, representing number of UEx 
...
%     text{i} = sprintf('F = %d GHz',(100*i));     
% end
% legend(text{1:2:u})
% xlabel("Number of UEs",'FontSize',Fontsize)
% ylabel("Profit (in $)",'FontSize',Fontsize)
% title("Impact of Computational Resources in MEC server",'FontSize',Fontsize+1)
% hold off
%%
% profit_genetic_avg = mean(profit_genetic);
% profit_sjoora_avg = mean(profit_sjoora);
% figure;
% hold on
% plot(x,profit_genetic_avg(start:iter))
% plot(x,profit_sjoora_avg(start:iter))
% xlabel("Number of UEs",'FontSize',Fontsize)
% ylabel("Profit (in $)",'FontSize',Fontsize)
...
% 
% hold off
% legend('Genetic','modified SJOORA')
...
if(cm_total<=1)  % if the there are still resources to be allocated, i.e.  there is no overshoot of computational resources then
    ...
    for i = 1:test_count % for all the MTs in test test count
        if(find(a(i,:)~=0)==rrh_min && e_m_sorted(i,1)<=e_m_sorted(index_min,1)) % find the the MTs which share the same RRH as the one with min. profit MT and have spectrum efficincy less than or equal to          
                ...
                fprintf('disallowed_UEs index = %d',disallowed_mts);
...
disp('Solving using Genetic Algorithm')
global  genetic_fast e N M Wm  B  F Ln fm_local Sm kappa omega pf pt qb qc

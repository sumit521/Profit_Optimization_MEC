...
%%
clear ; close all;
random_seed = 123;% default is 12345
rng(random_seed)
Fontsize = 12;
...
for u = 1:20 % number of iterations to average the result
...
for i = 1:z
    ...
    text{i} = sprintf('B = %d MHz',B_vector(i)/1e6);     
end
...
profit_greedy_avg = mean(profit_greedy);
profit_sbpso_avg = mean(profit_sbpso);
profit_dsbpso_avg = mean(profit_dsbpso);
figure;
hold on
box on
...
plot(x,profit_greedy_avg(start:iter),'-bs')
plot(x,profit_sbpso_avg(start:iter),'-gp')
plot(x,profit_dsbpso_avg(start:iter),'--gp')
xlabel("Number of UEs",'FontSize',Fontsize)
ylabel("Profit (in $)",'FontSize',Fontsize)
% title("Comparison of offloading strategies",'FontSize',Fontsize+1)

legend('SGA','FGA','Modified SJOORA','Greedy','SBPSO','DSBPSO','Location','Best')
hold off

saveas(gcf,'AlgorithmsComparison','epsc')
exportgraphics(gcf,'AlgorithmsComparison.png','Resolution',300)

% TIME COMPARISON
time_genetic_avg = mean(time_genetic(:,:,1));
time_fast_avg = mean(time_genetic(:,:,2));
time_sjoora_avg = mean(time_sjoora);
time_greedy_avg = mean(time_greedy);
time_sbpso_avg = mean(time_sbpso);
time_dsbpso_avg = mean(time_dsbpso);
figure;
hold on
box on
...
plot(x,time_greedy_avg(start:iter),'-bs')
plot(x,time_sbpso_avg(start:iter),'-gp')
plot(x,time_dsbpso_avg(start:iter),'--gp')
xlabel("Number of UEs",'FontSize',Fontsize)
ylabel(" Execution Time (per unit scale)",'FontSize',Fontsize)
% title("Comparison of offloading strategies",'FontSize',Fontsize+1)

legend('SGA','FGA','Modified SJOORA','Greedy','SBPSO','DSBPSO','Location','North')
hold off

saveas(gcf,'AlgorithmsComparisonTime','epsc')
exportgraphics(gcf,'AlgorithmsComparisonTime.png','Resolution',300)
save algocomparison_experiment.mat
